...
| `saved-query-catalog` | Frontmatter, tag, heading, reading, task, publish, and link metadata; `tags`, `frontmatter`, and `tasks` sidebar widgets; supported `outline-board` view; validator-supported `append-template`, `normalize-headings`, and `sort-lines` transforms | Executable saved-query language, query result views, saved dashboard/table views, computed rollups, and query scheduling |
